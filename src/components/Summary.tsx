...
const Summary: React.FC<SummaryProps> = ({ groupId }) => {
  ...
  useEffect(() => {
    if (groupId) {
      ...
      const groupExpenses = storedExpenses.filter(expense => expense.groupId === groupId);
  ...
  const calculateParticipantData = (expense: Expense) => {
    const totalWeight = expense.participants.reduce((acc, participant) => acc + participant.weight, 0);
    const totalAmount = parseFloat(String(expense.amount)) || 0;

    return expense.participants.map(participant => {
      const weightRatio = totalWeight > 0 ? participant.weight / totalWeight : 0;
      ...
      const isOwed = contribution > owedAmount ? (contribution - owedAmount).toFixed(2) : "0.00"; // Amount the participant should receive back
      const owed = contribution < owedAmount ? (owedAmount - contribution).toFixed(2) : "0.00"; // Amount they still need to contribute
  ...
  return (
    <div className="summary-container">
      {expenses.map(expense => (
        <div 
          key={expense.id} 
          className={`expense-item1 ${expandedExpenseId === expense.id ? "expanded" : ""}`}
        ...
        >
          <div className="expense-info">
            ...
            <div className="expense-details">
              ...
              <p className="expense-amount">${parseFloat(String(expense.amount)).toFixed(2)} · Participants: {expense.participants.length} · Date: {new Date(expense.date).toLocaleDateString()}</p>
            </div>
            <div className={`expense-arrow ${expandedExpenseId === expense.id ? "rotated" : ""}`}>
              <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="currentColor" viewBox="0 0 256 256">
            ...
            <div className="participant-table">
              <header className="table-header">
                <div className="header-row">
                  {["Participant", "Weight(%)", "Paid", "Owed", "Is Owed"].map((header, idx) => (
                    <div key={idx} className="header-cell">{header}</div>
                  ))}
                </div>
              </header>
              <div className="table-body">
                {calculateParticipantData(expense).map((participant, index) => (
                  <div key={index} className="body-row">
                    ...
                    <div className="body-cell">${(parseFloat(participant.contribution) || 0).toFixed(2)}</div>
