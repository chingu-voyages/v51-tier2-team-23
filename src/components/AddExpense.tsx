...
const AddExpense: React.FC<AddExpenseProps> = ({
  ...
  const handleAddExpense = () => {
    ...
    const totalContributions = participants.reduce((total, participant) => {
      const contributionValue = Number(participant.contribution.split(" ")[0]); // Extract numeric part of contribution
      return total + contributionValue;
    }, 0);

    // Update weights based on contributions if split evenly is selected
    const updatedParticipants = participants.map((participant) => {
      const contributionValue = Number(participant.contribution.split(" ")[0]);
      return {
        ...participant,
        weight: splitEvenly ? (contributionValue / totalContributions) * 100 : participant.weight,
      };
    });
    ...
    const newExpense = {
      ...
      currency,
      description,
      participants: updatedParticipants, // Use updated participants with weights
    ...
    setNewParticipantContribution(""); // Clear contribution amount
  ...
  const handleAddParticipant = () => {
    if (newParticipantName.trim() && newParticipantContribution.trim()) {
      const newParticipant: Participant = {
        ...
        avatarUrl: "https://cdn.usegalileo.ai/stability/e995b051-eff4-4ba3-964c-7ebd7b82b9dc.png", // Placeholder for avatar
        weight: 0,
      };
